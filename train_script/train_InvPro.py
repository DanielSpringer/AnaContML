#%%
import sys
sys.path.append('/gpfs/data/fs71925/dspringer1/Projects/InvPro/LuttingerWard_from_ML/src/')
import torch
...
def train():
    ...
    config = json.load(open('/gpfs/data/fs71925/dspringer1/Projects/InvPro/LuttingerWard_from_ML/_runs/confmod_graph_neural_network_MIT_w1000_n100.json'))[MODEL_NAME]
    print(config)
    ...
    PATH = "/gpfs/data/fs71925/dspringer1/Projects/InvPro/LuttingerWard_from_ML/_runs/saves_MIT/"
    # CONFIGURATION = f"{PATH}/{DATA_NAME}_1000/save_{config['MODEL_NAME']}_BS{config['batch_size']}_{datetime.datetime.now().date()}"
    CONFIGURATION = f"{PATH}/{DATA_NAME}_1000/save_{config['MODEL_NAME']}_BS{config['batch_size']}_{datetime.datetime.now().date()}"
